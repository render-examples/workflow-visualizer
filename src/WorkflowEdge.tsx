...
import { WorkflowEdgeConfig, WorkflowNodeConfig } from "./types";

// Arrow head component with proper rotation based on entry direction
function ArrowHead({ 
  x, y, entryDir, isActive 
}: { 
  x: number; y: number; entryDir: "up" | "down" | "left" | "right"; isActive: boolean 
}) {
  const size = 6;
...
function getConnectionPoints(from: WorkflowNodeConfig, to: WorkflowNodeConfig): { 
...
export function WorkflowEdge({ edge, nodes, isActive, isInFlow }: WorkflowEdgeProps) {
  ...
  const { fromPoint, toPoint, exitDir, entryDir } = getConnectionPoints(fromNode, toNode);
